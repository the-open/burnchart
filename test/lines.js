...
describe('lines', () => {
  it('actual', done => {
    ...
    const points = _.map(lines.actual(issues, 1, 6), ({ points }) => points);
  ...
  it('ideal', done => {
    ...
    const b = '2011-04-03T00:00:00Z';

    const line = lines.ideal(a, b, 4).slice(0, 3);
  ...
  it('trend', done => {
    ...
    const line = _.map(lines.trend.apply(null, opts), ({ points }) => Math.round(points));
